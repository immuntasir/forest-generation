...
if (nargin < 4) || isempty(cur_log_like)
    cur_log_like = log_like_fn(xx, varargin{:}); % zhu: compute logL(f)
end
...
if numel(prior) == D
    ...
    nu = reshape(prior'*randn(D, 1), size(xx)); % zhu: nu ~ N(0,chol_sig'*chol_sig).
end
hh = log(rand) + cur_log_like; % zhu: logy=logL(f)+logu

% Set up a bracket of angles and pick a first proposal.
% "phi = (theta'-theta)" is a change in angle.
if angle_range <= 0
    % Bracket whole ellipse with both edges at first proposed point
    phi = rand*2*pi; % zhu: theta~ Unif[0,2*pi]
    phi_min = phi - 2*pi;
    phi_max = phi;
else
...
while true
    ...
    xx_prop = xx*cos(phi) + nu*sin(phi); %zhu: f'=f*cos(theta)+nu*sin(theta)
    cur_log_like = log_like_fn(xx_prop, varargin{:}); % zhu: logL(f')
    if cur_log_like > hh
