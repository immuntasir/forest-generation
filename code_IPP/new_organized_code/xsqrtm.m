function [X, arg2, condest] = xsqrtm(A)
...
n = length(A);
[Q, T] = schur(A,'complex');  % T is complex Schur form. % zhu: this line is time consuming.
...
%     end
    %----------------------------------------------------------------        
    R=c_sqrtm_loop(T,sqrt(diag(T))); % replace line 41-51 by c code, 3 times faster.        
end

X = Q*R*Q'; % zhu: this line is time consuming. 
